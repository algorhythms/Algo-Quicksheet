...
\rih{Core clues}:
\begin{enumerate}
\item \pyinline{dist} is the distance from origin, not to destination.
...
\item During topological dfs, the $path$ holds \rih{extra} information. The $path$ holds the map of a list of ancestors with a given value, with the last one as the closest. 
\end{enumerate}
\begin{python}
